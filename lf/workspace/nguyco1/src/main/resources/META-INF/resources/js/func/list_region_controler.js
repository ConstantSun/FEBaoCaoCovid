...
var tag_image_path = 'https://nguyco.antoancovid.vn/images/response.png';
var image_share_path = 'https://nguyco.antoancovid.vn/images/share-icon.png';
var markerListLayer = [];
var isF0Mode = false;
class ListRegionControler {
    ...
    _getItemTagDiaPhuong(parent, item, fit_zoom_func, i) {
        ...
        const f0text = "<b>"+getF0(data_f0, parent + item) +"</b><br>F0-Công bố:"+ getF0(data_f0tk, parent + item);
		const f0textTK =getF0(data_f0tk, parent + item);
        const host = parent != '' ?
            `?p=${parent}&d=${item}` :
            `?p=${item}`;
        ...
        return `
                ...
                <a href="#" onclick="return ${fit_zoom_func};" title="F0-Công bố:${f0textTK}">${item}</a> (F0-Báo cáo:${f0text})
                ${content}                
                `;
    }
